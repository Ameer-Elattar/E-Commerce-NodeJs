...
exports.insert = [
    ...
    body("orders.*.products.*._id").isInt().withMessage("id of product must be number").custom((value) => {
        return productModel.findOne({ _id: value }).then((object) => {
          if (!object) throw new Error("this product is not exist");
        });
      }),
...
exports.update = [
    ...
    body("orders.*.products.*._id").optional().isInt().withMessage("id of product must be number").custom((value) => {
        return productModel.findOne({ _id: value }).then((object) => {
          if (!object) throw new Error("this product is not exist");
        });
      }),
